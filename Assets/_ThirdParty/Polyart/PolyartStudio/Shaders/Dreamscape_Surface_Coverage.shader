...
Shader "Polyart/Dreamscape Surface Coverage"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _CoverageNormalTexture_ST;
			float4 _CoverageTexture_ST;
			float4 _CoverageTint;
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv_CoverageTexture = IN.ase_texcoord7.xy * _CoverageTexture_ST.xy + _CoverageTexture_ST.zw;
				float2 uv_CoverageMask = IN.ase_texcoord7.xy * _CoverageMask_ST.xy + _CoverageMask_ST.zw;
				float saferPower22 = max( ( ( ( WorldNormal.y + _CoverageLevel ) * ( _CoverageFade * 5 ) ) + tex2D( _CoverageMask, uv_CoverageMask ).r ) , 0.0001 );
				...
				float3 tex2DNode76 = UnpackNormalScale( tex2D( _NormalTexture, uv_NormalTexture ), 1.0f );
				float2 uv_CoverageNormalTexture = IN.ase_texcoord7.xy * _CoverageNormalTexture_ST.xy + _CoverageNormalTexture_ST.zw;
				float3 tex2DNode79 = UnpackNormalScale( tex2D( _CoverageNormalTexture, uv_CoverageNormalTexture ), 1.0f );
				#ifdef _BLENDNORMALSON_ON
				float3 staticSwitch80 = BlendNormal( tex2DNode76 , tex2DNode79 );
				#else
				...
				float2 uv_CoverageSmoothnessTexture = IN.ase_texcoord7.xy * _CoverageSmoothnessTexture_ST.xy + _CoverageSmoothnessTexture_ST.zw;
				...
				float3 Albedo = staticSwitch1.rgb;
				float3 Normal = vNormal82;
				float3 Emission = vEmissive95.rgb;
				float3 Specular = 0.5;
				float Metallic = vMetallic72.r;
				float Smoothness = vSmoothness51.r;
				float Occlusion = 1;
				float Alpha = 1;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 temp_output_5_0 = ( _AlbedoTint * tex2D( _AlbedoTexture, uv_AlbedoTexture ) );
				float2 uv_CoverageTexture = IN.ase_texcoord2.xy * _CoverageTexture_ST.xy + _CoverageTexture_ST.zw;
...
/*ASEBEGIN
Version=18800
756;73;2305;1472;1305.527;828.8405;1;True;True
...
WireConnection;110;3;74;0
WireConnection;110;4;59;0
ASEEND*/
//CHKSM=5513FC17FABC962422CD0A6BB30243AFCEF9446C